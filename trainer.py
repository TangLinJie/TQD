...
class Trainer:
    ...
    def update_learning_rate(self, optimizer, epoch, step):
        ...
        if hasattr(self.experiment.train.scheduler.learning_rate, 'set_lr'):
            self.current_lr = self.experiment.train.scheduler.learning_rate.set_lr(optimizer, step, (self.experiment.train.epochs+1)*self.total)[0]
        else:
    ...
    def train(self):
        ...
        if self.experiment.train.checkpoint:
            ...
            self.experiment.train.checkpoint.restore_model_v2(
                model, self.device, self.logger)
        ...
        if self.experiment.train.checkpoint:
            self.experiment.train.checkpoint.restore_optimizer(optimizer, self.device, self.logger)
            self.experiment.train.checkpoint.restore_learning_rate(self.experiment.train.scheduler.learning_rate, self.device, self.logger)

        # for mixed-precision training with apex
        # model_model, optimizer = amp.initialize(model.model, optimizer, opt_level='O1')
        ...
        # model.model = parallelize(model.model, self.experiment.distributed, self.experiment.local_rank)
        ...
        while True:
            ...
            for batch in train_data_loader:
                ...
                self.model_saver.maybe_save_model_optimizer(model, optimizer, \
                                        self.experiment.train.scheduler.learning_rate, epoch, self.steps, self.logger)
            ...
            if epoch > self.experiment.train.epochs:
                ...
                self.model_saver.save_checkpoint_v2(model, optimizer, self.experiment.train.scheduler.learning_rate, 'final')
                if self.experiment.validation:
                    self.validate(validation_loaders, model, epoch, self.steps)
