...
class ModelSaver(Configurable):
    ...
    def maybe_save_model_optimizer(self, model, optimizer, learning_rate, epoch, step, logger):
        if step % self.save_interval == 0 or self.monitor.get_signal() is not None:
            save_dict = {'model':model.state_dict(), 'optimizer':optimizer.state_dict(), 'base_lr':learning_rate.base_lr}
            checkpoint_name = self.make_checkpoint_name('model', epoch, step)
            os.makedirs(self.dir_path, exist_ok=True)
            torch.save(save_dict, os.path.join(self.dir_path, checkpoint_name))
            logger.report_time('Saving ')
            logger.iter(step)

    def save_checkpoint_v2(self, model, optimizer, learning_rate, name):
        os.makedirs(self.dir_path, exist_ok=True)
        save_dict = {'model':model.state_dict(), 'optimizer':optimizer.state_dict(), 'base_lr':learning_rate.base_lr}
        torch.save(save_dict, os.path.join(self.dir_path, name))
