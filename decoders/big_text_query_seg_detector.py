...
class BigTextQuerySegDetector(nn.Module):
    def __init__(self,
                 ...
                 *args, **kwargs):
        super(BigTextQuerySegDetector, self).__init__()
        self.k = k
        self.serial = serial
        ...
        self.kernel_pred = nn.Linear(inner_channels, inner_channels*self.kernel_size[0]*self.kernel_size[1])
            ...
            for img_idx in range(kernel_params.shape[1]):
                ...
                for query_idx in range(kernel_params.shape[2]):
                    query_mask = F.conv2d(fuse[img_idx].unsqueeze(0), kernel_params[layer_idx][img_idx][query_idx].view(\
                                            1, self.inner_channels, self.kernel_size[0], self.kernel_size[1]),\
