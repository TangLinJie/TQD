...
class MultiLayersBCELoss(nn.Module):
    def __init__(self, eps=1e-6, l1_scale=10, bce_scale=5, field_scale=0.01, layer_scale=[1., 1., 1., 1., 1., 1.]):
        super(MultiLayersBCELoss, self).__init__()
        from .dice_loss import DiceLoss
        from .l1_loss import MaskL1Loss
        from .balance_cross_entropy_loss import BalanceCrossEntropyLoss
        self.dice_loss = DiceLoss(eps=eps)
        ...
        self.field_loss = None

    def forward(self, pred, batch):
        num_layers = pred['binary'].shape[0]
        metrics = {}
        total_loss = 0.
        for layer_idx in range(num_layers-1, -1, -1):
            bce_loss = self.bce_loss(pred['binary'][layer_idx], batch['gt'], batch['mask'])
            metrics['bce_loss_{}'.format(layer_idx)] = bce_loss
            if 'thresh' in pred:
                l1_loss, l1_metric = self.l1_loss(pred['thresh'][layer_idx], batch['thresh_map'], batch['thresh_mask'])
                dice_loss = self.dice_loss(pred['thresh_binary'][layer_idx], batch['gt'], batch['mask'])
                metrics['thresh_loss_{}'.format(layer_idx)] = dice_loss
                loss = dice_loss + self.l1_scale * l1_loss + bce_loss * self.bce_scale
                metrics.update({'l1_loss_{}'.format(layer_idx): l1_metric['l1_loss']})
            else:
                loss = bce_loss
            if 'field' in pred:
                field_loss = self.field_loss(pred['field'][layer_idx], batch['field_map'], batch['field_weight'])
                metrics['field_loss_{}'.format(layer_idx)] = field_loss
                loss = loss + (field_loss * self.field_scale)
            total_loss += (self.layer_scale[layer_idx] * loss)
