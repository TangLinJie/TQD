...
class ImageDataset(data.Dataset, Configurable):
        ...
        for i in range(len(self.data_dir)):
            ...
            if self.is_training:
                ...
                if 'SynthText' in self.data_list[i]:
                    gt_path=[self.data_dir[i]+'/SynthText_GT_E2E/'+timg.strip()+'.txt' for timg in image_list]
                    image_path=[self.data_dir[i]+'/'+timg.strip() for timg in image_list]
                elif 'synthtext150k' in self.data_list[i]:
                    gt_path=[self.data_dir[i]+'/'+timg.strip().replace('train_images', 'train_gts')+'.txt' for timg in image_list]
                    image_path=[self.data_dir[i]+'/'+timg.strip() for timg in image_list]
                elif 'mlt17' in self.data_list[i] and 'train' in self.data_list[i]:
                    image_path=[self.data_dir[i]+'/train_images/'+timg.strip() for timg in image_list]
                    gt_path=[self.data_dir[i]+'/train_gts/'+'gt_'+timg.strip().split('.')[0]+'.txt' for timg in image_list]
                elif 'mlt17' in self.data_list[i] and 'val' in self.data_list[i]:
                    image_path=[self.data_dir[i]+'/val_images/'+timg.strip() for timg in image_list]
                ...
                elif 'mlt17' in self.data_dir[i] and 'test' in self.data_list[i]:
                    gt_path = []
                else:
    ...
    def load_ann(self):
        res = []
        for gt_indx in range(len(self.gt_paths)):
            gt = self.gt_paths[gt_indx]
            lines = []
            ...
            for line in reader:
                ...
                label = parts[-1]
                if 'synthtext150k' in gt:
                    parts = line.strip().split('\t')
                    label = parts[-1]
                if 'TD' in self.data_dir[0] and label == '1':
                    label = '###'
                line = [i.strip('\ufeff').strip('\xef\xbb\xbf') for i in parts]
                if 'icdar' in self.data_dir[0] or 'SynthText' in self.data_dir[0] or 'mlt17' in gt:
    ...
    def __getitem__(self, index, retry=0):
        ...
        data['image'] = img
        if not self.is_training and ('mlt17' in image_path and 'test' in image_path):
            target = []
        else:
